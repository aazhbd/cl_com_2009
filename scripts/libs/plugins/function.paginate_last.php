...
function smarty_function_paginate_last($params, &$smarty) {
    ...
    $_nextitem = ($_total % $_limit > 0) ? $_total - ( $_total % $_limit ) + 1 : $_total - $_limit + 1;
    ...
    $_url .= $_nextitem;
        ...
        if(isset($params['rewrite'])) {
            $_rewrite = isset($params['rewrite']) ? $params['rewrite'] : SmartyPaginate::getFirstText($_id);
            $_rewrite = preg_replace("/@@@/is",$_nextitem,$_rewrite);
            return '<a href="' . str_replace('&','&amp;', $_rewrite) . '">' . $_text . '</a>';
        }
