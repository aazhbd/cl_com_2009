...
function smarty_function_paginate_middle($params, &$smarty) {
        ...
        if($_item != $_curr_item) {
            ...
            $_this_url .= SmartyPaginate::getUrlVar($_id) . '=' . $_item;
                if(isset($params['rewrite'])) {
                    $_this_url = preg_replace("/@@@/is",$_item,$params['rewrite']);
                    $_ret .= $_link_prefix . ' <a href="' . str_replace('&', '&amp;', $_this_url) . '">' . $_text . '</a> ' . $_link_suffix;
                }
                else {
    ...
    //return ceil(SmartyPaginate::getLastItem($_id) / SmartyPaginate::getLimit($_id));
    return $_ret;
